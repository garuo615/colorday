...
function minutesToTimeStr(minutes: number): string {
  const c = Math.max(0, Math.min(1439, Math.floor(minutes)));
...
export const AddTaskModal: React.FC<Props> = ({
  ...
  const handleSave = () => {
    ...
    if (existingTasks.some(t => startTime < t.endTime && endTime > t.startTime)) {
      setError('既存のタスクと時間が重複しています');
      return;
    }
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <input type="checkbox" id="add-useCustom" checked={useCustom}
                onChange={e => setUseCustom(e.target.checked)}
                style={{ cursor: 'pointer', width: 16, height: 16 }}
              />
              <label htmlFor="add-useCustom" style={{ fontSize: 14, color: '#4b5563', cursor: 'pointer' }}>
                カスタムカテゴリを使う
              </label>
            </div>
            {useCustom && (
