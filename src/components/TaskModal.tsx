...
export const TaskModal: React.FC<Props> = ({ initialStartTime, initialEndTime, onSave, onCancel }) => {
  ...
  const handleSave = () => {
    ...
    onSave(category, taskName.trim(), timeStrToTs(startStr, initialStartTime), timeStrToTs(endStr, initialEndTime));
  };
        ...
        <div style={{
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <input type="checkbox" id="tm-useCustom" checked={useCustom}
                onChange={e => setUseCustom(e.target.checked)}
                style={{ cursor: 'pointer', width: 16, height: 16 }}
              />
              <label htmlFor="tm-useCustom" style={{ fontSize: 14, color: '#4b5563', cursor: 'pointer' }}>
                カスタムカテゴリを使う
              </label>
            </div>
            {useCustom && (
